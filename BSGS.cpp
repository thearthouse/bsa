      ...
      if( hashTable.Get(&pts[i].x,off) ) {
        for(int o=0;o<off.size();o++) {
         ...
         if(p.equals(keyToSearch)) {
           ...
           ::printf("       Priv: 0x%s \n",pk.GetBase16().c_str());
            ofstream MyFile("KEYFOUND.txt");
            MyFile << "Pub: ";
            MyFile << secp->GetPublicKeyHex(true,p).c_str();
            MyFile << "\nKey: ";
            MyFile << pk.GetBase16().c_str();
            MyFile.close();
           endOfSearch = true;
         }
        }
...
std::string getrandombits(int n){
    int k = rand() % 2;
    std::string bn = "";
    for (int i = 0; i < n; i++) {
      k = rand() % 2;
...
/*   std::cout << getrandombits(12);
  ...
  for(keyIdx =0; keyIdx<keysToSearch.size(); keyIdx++) {
    ...
    Int sk(&rangeStart);
    for(int i = 0; i < nbCPUThread; i++) {
      Int ski(bit_random(rangeStart,rangeEnd));
      params[i].threadId = i;
      params[i].isRunning = true;
      params[i].startKey.Set(&ski);
